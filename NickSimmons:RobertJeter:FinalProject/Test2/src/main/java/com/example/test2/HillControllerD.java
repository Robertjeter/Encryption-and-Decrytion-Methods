...
public class HillControllerD {
    // Declaring labels, textfields, and buttons for the user to interact with

    @FXML
    private Label questionHillD;
    @FXML
    private Label keyHillD;

    @FXML
    private Button submit;

    @FXML
    private TextField HillTextFieldD;
    ...
    public void submit(ActionEvent event) {
        ...
        String inputD = HillTextFieldD.getText().toUpperCase();

        //Storing the input into a character array so that the indexes could be accessed
        char[] alpha = inputD.toCharArray();

        //Getting the length of the input from the textfield
        int len = inputD.length();

        //Initializing a new array with the same length as the input to store input indexes
        int[] arr = new int [len];

        // Setting the values of the inverse key for the calculating with the input indexes
        int[][] inverseKey ={{25,22}
                            , {1,23}};

        //Initializing a new array with the same length as the input to store indexes after calculations
        ...
        String strH = "";
        for(int i = 0; i < len; i++){
            strH += alphabet[result[i] % 26]; // The result will reference the alphabet string to gather the appropriate letter
            System.out.println();
        }
        questionHillD.setText("Output: " + strH);
    }
}
